...
def compression(tree):
    ...
    print('{:d} points out of {:d} for full grid'.format(nleaves,tot))
...
def start_refine(tree):
    ...
    return len(total)
def refine(tree,tol=.8,eps=.01,**kargs):
    tree.walk(leaf_func = lambda x: refinement_check(x,tol=tol,eps=eps,**kargs))
